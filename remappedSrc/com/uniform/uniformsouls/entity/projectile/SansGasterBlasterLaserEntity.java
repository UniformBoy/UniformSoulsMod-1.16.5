...
public class SansGasterBlasterLaserEntity extends ThrownEntity {
    ...
    public SansGasterBlasterLaserEntity(EntityType<? extends SansGasterBlasterLaserEntity> entityType, World world) {
        super(entityType, world);
    }

    @Environment(EnvType.CLIENT)
    public SansGasterBlasterLaserEntity(World world, double x, double y, double z, int id, UUID uuid) {
        super(UniformSouls.SANS_GASTER_BLASTER_LASER_ENTITY, world);
        setPosition(x, y, z);
    ...
    public void onEntityHit(EntityHitResult entityHitResult) {
        if (!this.world.isClient) {
            ...
            if (entity2 instanceof LivingEntity) {
                ...
                bl2 = entity.damage(UniformSouls.GASTER_BLASTER_SANS_DAMAGE, 5.0F);
                if (bl2) {
                    if (entity.isAlive()) {
    ...
    public void onCollision(HitResult hitResult) {
        ...
        if (hitResult.getType() != HitResult.Type.ENTITY || !((EntityHitResult)hitResult).getEntity().isPartOf(entity)) {
            if (!this.world.isClient) {
                world.createExplosion(this, getX(), getY(), getZ(), 4, Explosion.DestructionType.NONE);
                ...
                areaEffectCloudEntity.addEffect(new StatusEffectInstance(UniformSouls.SANSKARMADAMAGE1, 20, 0));
                if (!list.isEmpty()) {
