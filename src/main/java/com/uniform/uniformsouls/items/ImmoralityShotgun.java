...
public class ImmoralityShotgun extends SwordItem {

    public ImmoralityShotgun(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.isSneaking()) {
            ...
            world.playSound(null, user.getX(), user.getY(), user.getZ(), UniformSouls.JUSTICE_PISTOL_GUNSHOT_EVENT, SoundCategory.PLAYERS, 2.5F, 0.5F); // plays a globalSoundEvent
            user.getItemCooldownManager().set(this, 150);

            if (!world.isClient) {
                ImmoralityBulletEntity proj = new ImmoralityBulletEntity(UniformSouls.IMMORALITY_BULLET_ENTITY, world);
            ...
            ItemStack itemStack = new ItemStack(ModItems.IMMORALITY_SOUL);
