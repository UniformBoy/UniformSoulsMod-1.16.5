...
public class BraveryToughGlove extends SwordItem {

    public BraveryToughGlove(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.isSneaking()) {
            ...
            world.playSound(null, user.getX(), user.getY(), user.getZ(), UniformSouls.GASTERBLASTERSOUNDEFFECT_TRIM_EVENT, SoundCategory.PLAYERS, 1.0F, 2.0F); // plays a globalSoundEvent
            user.getItemCooldownManager().set(this, 50);

            if (!world.isClient) {
                BraveryGloveEntity proj = new BraveryGloveEntity(UniformSouls.BRAVERY_GLOVE_ENTITY, world);
            ...
            ItemStack itemStack = new ItemStack(ModItems.BRAVERY_SOUL);
