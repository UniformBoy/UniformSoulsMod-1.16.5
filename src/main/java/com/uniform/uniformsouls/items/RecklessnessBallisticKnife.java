...
public class RecklessnessBallisticKnife extends SwordItem {

    public RecklessnessBallisticKnife(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.isSneaking()) {
            ...
            world.playSound(null, user.getX(), user.getY(), user.getZ(), UniformSouls.PATIENCE_STRING_THROW_EVENT, SoundCategory.PLAYERS, 1.0F, 0.5F); // plays a globalSoundEvent
            user.getItemCooldownManager().set(this, 150);

            if (!world.isClient) {
                RecklessnessStringEntity proj = new RecklessnessStringEntity(UniformSouls.RECKLESSNESS_STRING_ENTITY, world);
            ...
            ItemStack itemStack = new ItemStack(ModItems.RECKLESSNESS_SOUL);
