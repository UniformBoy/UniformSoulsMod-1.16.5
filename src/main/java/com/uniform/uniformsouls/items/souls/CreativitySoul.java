...
public class CreativitySoul extends Item {

    public CreativitySoul(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity playerEntity, Hand hand) {
        ...
        if (!playerEntity.isSneaking()) {
            ...
            playerEntity.playSound(SoundEvents.BLOCK_NOTE_BLOCK_BIT, 2.0F, 1.0F/(RANDOM.nextFloat()*.4F + .8F));
            playerEntity.getItemCooldownManager().set(this, 60);
            playerEntity.incrementStat(Stats.USED.getOrCreateStat(this));

            return TypedActionResult.success(playerEntity.getStackInHand(hand));
        } else {
            ItemStack itemStack = new ItemStack(ModItems.CREATIVITY_BATTLE_AXE);
    ...
    {
        //Basic Info
        tooltip.add(new TranslatableText("You Are Filled With Creativity").formatted(Formatting.LIGHT_PURPLE));
