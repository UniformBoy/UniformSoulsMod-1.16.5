...
public class WrathSoul extends Item {

    public WrathSoul(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity playerEntity, Hand hand) {
        ...
        if (!playerEntity.isSneaking()) {
            ...
            playerEntity.playSound(SoundEvents.BLOCK_NOTE_BLOCK_HARP, 2.0F, 1.0F/(RANDOM.nextFloat()*.4F - .8F));
            playerEntity.getItemCooldownManager().set(this, 10);
            playerEntity.incrementStat(Stats.USED.getOrCreateStat(this));

            return TypedActionResult.success(playerEntity.getStackInHand(hand));
        } else {
            ItemStack itemStack = new ItemStack(ModItems.WRATH_DOUBLE_FRYING_PAN);
    ...
    {
        //Basic Info
        tooltip.add(new TranslatableText("§a§n§m§o§keee §r§a§n§m§oWrath §keee"));
