...
public class Spear1 extends SwordItem {

    public Spear1(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.isSneaking()) {
            ...
            world.playSound(null, user.getX(), user.getY(), user.getZ(), UniformSouls.WOOSH_NORMAL_OUT_1_EVENT, SoundCategory.PLAYERS, 1.0F, 0.9F); // plays a globalSoundEvent
            user.getItemCooldownManager().set(this, 50);

            if (!world.isClient) {
                SpearProjEntity proj = new SpearProjEntity(UniformSouls.SPEAR_PROJ_ENTITY, world);
            ...
            ItemStack itemStack = new ItemStack(ModItems.SPEAR_MONSTER_SOUL);
