...
public class ApathyCarelessJournal extends SwordItem {

    public ApathyCarelessJournal(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.isSneaking()) {
            ...
            world.playSound(null, user.getX(), user.getY(), user.getZ(), UniformSouls.PERSEVERANCE_LASER_BLAST_EVENT, SoundCategory.PLAYERS, 1.0F, 0.2F); // plays a globalSoundEvent
            user.getItemCooldownManager().set(this, 150);

            if (!world.isClient) {
                ApathyLaserEntity proj = new ApathyLaserEntity(UniformSouls.APATHY_LASER_ENTITY, world);
            ...
            ItemStack itemStack = new ItemStack(ModItems.APATHY_SOUL);
