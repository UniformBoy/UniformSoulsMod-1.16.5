...
public class EnergyBeam1 extends SwordItem {

    public EnergyBeam1(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.isSneaking()) {
            ...
            world.playSound(null, user.getX(), user.getY(), user.getZ(), UniformSouls.WOOSH_NORMAL_OUT_1_EVENT, SoundCategory.PLAYERS, 1.0F, 0.85F); // plays a globalSoundEvent
            world.playSound(null, user.getX(), user.getY(), user.getZ(), SoundEvents.ENTITY_LIGHTNING_BOLT_THUNDER, SoundCategory.PLAYERS, 2.5F, 0.85F); // plays a globalSoundEvent
            user.getItemCooldownManager().set(this, 120);

            if (!world.isClient) {
                EnergyBeamProjEntity proj = new EnergyBeamProjEntity(UniformSouls.ENERGY_BEAM_PROJ_ENTITY, world);
            ...
            ItemStack itemStack = new ItemStack(ModItems.METALIC_MONSTER_SOUL);
