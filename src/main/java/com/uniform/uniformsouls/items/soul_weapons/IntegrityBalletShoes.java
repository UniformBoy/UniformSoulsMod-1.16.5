...
public class IntegrityBalletShoes extends SwordItem {

    public IntegrityBalletShoes(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.isSneaking()) {
            ...
            world.playSound(null, user.getX(), user.getY(), user.getZ(), UniformSouls.INTEGRITY_SAWBLADE_FIRE_EVENT, SoundCategory.PLAYERS, 1.0F, 1F); // plays a globalSoundEvent
            user.getItemCooldownManager().set(this, 50);

            if (!world.isClient) {
                IntegritySawbladeEntity proj = new IntegritySawbladeEntity(UniformSouls.INTEGRITY_SAWBLADE_ENTITY, world);
            ...
            ItemStack itemStack = new ItemStack(ModItems.INTEGRITY_SOUL);
