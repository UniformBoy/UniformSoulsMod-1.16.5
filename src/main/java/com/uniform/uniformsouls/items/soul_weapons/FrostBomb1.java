...
public class FrostBomb1 extends SwordItem {

    public FrostBomb1(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.isSneaking()) {
            ...
            world.playSound(null, user.getX(), user.getY(), user.getZ(), UniformSouls.WOOSH_NORMAL_OUT_1_EVENT, SoundCategory.PLAYERS, 1.0F, 0.65F); // plays a globalSoundEvent
            world.playSound(null, user.getX(), user.getY(), user.getZ(), SoundEvents.ENTITY_SNOWBALL_THROW, SoundCategory.PLAYERS, 1.0F, 0.65F); // plays a globalSoundEvent
            user.getItemCooldownManager().set(this, 50);

            if (!world.isClient) {
                FrostBomb1Entity proj = new FrostBomb1Entity(UniformSouls.FROST_BOMB_ENTITY, world);
            ...
            ItemStack itemStack = new ItemStack(ModItems.FROST_MONSTER_SOUL);
