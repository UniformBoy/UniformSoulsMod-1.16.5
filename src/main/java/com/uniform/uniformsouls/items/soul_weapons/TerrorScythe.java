...
public class TerrorScythe extends SwordItem {

    public TerrorScythe(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.isSneaking()) {
            ...
            world.playSound(null, user.getX(), user.getY(), user.getZ(), UniformSouls.FEAR_SCYTHE_SLASH_1_EVENT, SoundCategory.PLAYERS, 1.3F, 0.5F); // plays a globalSoundEvent
            user.getItemCooldownManager().set(this, 150);

            if (!world.isClient) {
                TerrorScytheSlashEntity proj = new TerrorScytheSlashEntity(UniformSouls.TERROR_SCYTHE_SLASH_ENTITY, world);
            ...
            ItemStack itemStack = new ItemStack(ModItems.TERROR_SOUL);
