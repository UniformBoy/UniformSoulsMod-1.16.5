...
public class DishonorSturdyGlove extends SwordItem {

    public DishonorSturdyGlove(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.isSneaking()) {
            ...
            world.playSound(null, user.getX(), user.getY(), user.getZ(), UniformSouls.GASTERBLASTERSOUNDEFFECT_TRIM_EVENT, SoundCategory.PLAYERS, 1.0F, 1.5F); // plays a globalSoundEvent
            user.getItemCooldownManager().set(this, 150);

            if (!world.isClient) {
                DishonorGloveEntity proj = new DishonorGloveEntity(UniformSouls.DISHONOR_GLOVE_ENTITY, world);
            ...
            ItemStack itemStack = new ItemStack(ModItems.DISHONOR_SOUL);
