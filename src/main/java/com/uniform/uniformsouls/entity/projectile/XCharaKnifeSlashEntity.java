...
public class XCharaKnifeSlashEntity extends ThrownEntity implements DefaultSoulProjectile {

    public static final Identifier SPAWN_PACKET = new Identifier(UniformSouls.MOD_ID, "xchara_knife_slash");
    private Object PlayerEntity;

    public XCharaKnifeSlashEntity(EntityType<? extends XCharaKnifeSlashEntity> entityType, World world) {
        super(entityType, world);
    }

    @Environment(EnvType.CLIENT)
    public XCharaKnifeSlashEntity(World world, double x, double y, double z, int id, UUID uuid) {
        super(UniformSouls.XCHARA_KNIFE_SLASH_ENTITY, world);
        setPosition(x, y, z);
    ...
    public void tick() {
        ...
        if (this.world.isClient) {
            this.world.addParticle(UniformSouls.FLASH_OVERWRITE_DETERMINATION, this.getX(), this.getY(), this.getZ(), 0.0D, 0.0D, 0.0D);
        }
    ...
    public void onCollision(HitResult hitResult) {
        ...
        if (hitResult.getType() != HitResult.Type.ENTITY || !((EntityHitResult)hitResult).getEntity().isPartOf(entity)) {
            if (!this.world.isClient) {
                ...
                areaEffectCloudEntity.setParticleType(UniformSouls.SOUL_OVERWRITE_DETERMINATION);
                ...
                world.addParticle(UniformSouls.FLASH_OVERWRITE_DETERMINATION, this.getX(), this.getY(), this.getZ(), 0.0D, 0.0D, 0.0D);
                this.remove();
            }
