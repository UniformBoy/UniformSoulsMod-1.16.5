...
public class EnergyBeamProjEntity extends ThrownEntity implements DefaultSoulProjectile {

    public static final Identifier SPAWN_PACKET = new Identifier(UniformSouls.MOD_ID, "energy_beam_proj");
    private Object PlayerEntity;

    public EnergyBeamProjEntity(EntityType<? extends EnergyBeamProjEntity> entityType, World world) {
        super(entityType, world);
    }

    @Environment(EnvType.CLIENT)
    public EnergyBeamProjEntity(World world, double x, double y, double z, int id, UUID uuid) {
        super(UniformSouls.ENERGY_BEAM_PROJ_ENTITY, world);
        setPosition(x, y, z);
    ...
    public void onCollision(HitResult hitResult) {
        ...
        if (hitResult.getType() != HitResult.Type.ENTITY || !((EntityHitResult)hitResult).getEntity().isPartOf(entity)) {
            if (!this.world.isClient) {
                ...
                areaEffectCloudEntity.setRadiusGrowth((2.0F - areaEffectCloudEntity.getRadius()) / (float)areaEffectCloudEntity.getDuration());
                areaEffectCloudEntity.addEffect(new StatusEffectInstance(StatusEffects.WITHER, 200, 1));
                areaEffectCloudEntity.addEffect(new StatusEffectInstance(StatusEffects.BLINDNESS, 200, 4));
                areaEffectCloudEntity.addEffect(new StatusEffectInstance(StatusEffects.GLOWING, 200, 0));
                if (!list.isEmpty()) {
                ...
                world.playSound(null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_LIGHTNING_BOLT_THUNDER, SoundCategory.PLAYERS, 0.8F, 0.85F); // plays a globalSoundEvent
