...
public class ImmoralityBulletEntity extends ThrownEntity {
    ...
    public ImmoralityBulletEntity(EntityType<? extends ImmoralityBulletEntity> entityType, World world) {
        super(entityType, world);
    }

    @Environment(EnvType.CLIENT)
    public ImmoralityBulletEntity(World world, double x, double y, double z, int id, UUID uuid) {
        super(UniformSouls.IMMORALITY_BULLET_ENTITY, world);
        setPosition(x, y, z);
    ...
    public void tick() {
        ...
        if (this.world.isClient) {
            this.world.addParticle(UniformSouls.FLASH_IMMORALITY, this.getX(), this.getY(), this.getZ(), 0.0D, 0.0D, 0.0D);
        }
    ...
    public void onEntityHit(EntityHitResult entityHitResult) {
        if (!this.world.isClient) {
            ...
            if (entity2 instanceof LivingEntity) {
                ...
                bl2 = entity.damage(UniformSouls.IMMORALITY_BULLET_DAMAGE, 5.0F);
                if (bl2) {
                    if (entity.isAlive()) {
    ...
    public void onCollision(HitResult hitResult) {
        ...
        if (hitResult.getType() != HitResult.Type.ENTITY || !((EntityHitResult)hitResult).getEntity().isPartOf(entity)) {
            if (!this.world.isClient) {
                ...
                areaEffectCloudEntity.setParticleType(UniformSouls.SOUL_IMMORALITY);
                ...
                areaEffectCloudEntity.addEffect(new StatusEffectInstance(UniformSouls.JUSDEBUFFEFFECT1, 200, 1));
                if (!list.isEmpty()) {
                ...
                world.addParticle(UniformSouls.FLASH_IMMORALITY, this.getX(), this.getY(), this.getZ(), 0.0D, 0.0D, 0.0D);
                this.remove();
            }
