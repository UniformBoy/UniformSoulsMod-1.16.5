...
public class FrostBomb1Entity extends ThrownEntity implements DefaultSoulProjectile {

    public static final Identifier SPAWN_PACKET = new Identifier(UniformSouls.MOD_ID, "frost_bomb_1");
    private Object PlayerEntity;

    public FrostBomb1Entity(EntityType<? extends FrostBomb1Entity> entityType, World world) {
        super(entityType, world);
    }

    @Environment(EnvType.CLIENT)
    public FrostBomb1Entity(World world, double x, double y, double z, int id, UUID uuid) {
        super(UniformSouls.FROST_BOMB_ENTITY, world);
        setPosition(x, y, z);
    ...
    public void onCollision(HitResult hitResult) {
        ...
        if (hitResult.getType() != HitResult.Type.ENTITY || !((EntityHitResult)hitResult).getEntity().isPartOf(entity)) {
            if (!this.world.isClient) {
                ...
                areaEffectCloudEntity.setParticleType(ParticleTypes.SPIT);
                ...
                areaEffectCloudEntity.addEffect(new StatusEffectInstance(UniformSouls.CHILLEDEFFECT1, 200, 2));
                if (!list.isEmpty()) {
