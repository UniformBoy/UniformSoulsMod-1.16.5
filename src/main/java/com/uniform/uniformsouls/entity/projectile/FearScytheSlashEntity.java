...
public class FearScytheSlashEntity extends ThrownEntity implements DefaultSoulProjectile {

    public static final Identifier SPAWN_PACKET = new Identifier(UniformSouls.MOD_ID, "fear_scythe_slash");
    private Object PlayerEntity;

    public FearScytheSlashEntity(EntityType<? extends FearScytheSlashEntity> entityType, World world) {
        super(entityType, world);
    }

    @Environment(EnvType.CLIENT)
    public FearScytheSlashEntity(World world, double x, double y, double z, int id, UUID uuid) {
        super(UniformSouls.FEAR_SCYTHE_SLASH_ENTITY, world);
        setPosition(x, y, z);
    ...
    public void onEntityHit(EntityHitResult entityHitResult) {
        if (!this.world.isClient) {
            ...
            if (entity2 instanceof LivingEntity) {
                ...
                bl2 = entity.damage(UniformSouls.FEAR_SCYTHE_SLASH_DAMAGE, 5.0F);
                if (bl2) {
                    if (entity.isAlive()) {
    ...
    public void onCollision(HitResult hitResult) {
        ...
        if (hitResult.getType() != HitResult.Type.ENTITY || !((EntityHitResult)hitResult).getEntity().isPartOf(entity)) {
            if (!this.world.isClient) {
                ...
                areaEffectCloudEntity.setParticleType(UniformSouls.SOUL_FEAR);
                ...
                areaEffectCloudEntity.addEffect(new StatusEffectInstance(UniformSouls.FEADAMAGE1, 20, 0));
                ...
                areaEffectCloudEntity.setRadiusGrowth((5.0F - areaEffectCloudEntity.getRadius()) / (float)areaEffectCloudEntity.getDuration());
                areaEffectCloudEntity.addEffect(new StatusEffectInstance(UniformSouls.SCARED, 100, 4));
