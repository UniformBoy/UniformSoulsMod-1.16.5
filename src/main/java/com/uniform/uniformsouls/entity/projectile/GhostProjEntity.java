...
public class GhostProjEntity extends ThrownEntity implements DefaultSoulProjectile {

    public static final Identifier SPAWN_PACKET = new Identifier(UniformSouls.MOD_ID, "ghost_proj");
    private Object PlayerEntity;

    public GhostProjEntity(EntityType<? extends GhostProjEntity> entityType, World world) {
        super(entityType, world);
    }

    @Environment(EnvType.CLIENT)
    public GhostProjEntity(World world, double x, double y, double z, int id, UUID uuid) {
        super(UniformSouls.GHOST_PROJ_ENTITY, world);
        setPosition(x, y, z);
    ...
    public void tick() {
        ...
        if (this.world.isClient) {
            ...
            this.world.addParticle(ParticleTypes.CLOUD, this.getX(), this.getY(), this.getZ(), 0.0D, 0.0D, 0.0D);
        }
    ...
    public void onCollision(HitResult hitResult) {
        ...
        if (hitResult.getType() != HitResult.Type.ENTITY || !((EntityHitResult)hitResult).getEntity().isPartOf(entity)) {
            if (!this.world.isClient) {
                ...
                areaEffectCloudEntity.addEffect(new StatusEffectInstance(StatusEffects.INSTANT_DAMAGE, 200, 0));
                areaEffectCloudEntity.addEffect(new StatusEffectInstance(StatusEffects.LEVITATION, 100, 1));
                if (!list.isEmpty()) {
