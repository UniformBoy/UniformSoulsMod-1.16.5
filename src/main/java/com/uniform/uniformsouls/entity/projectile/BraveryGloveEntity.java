...
public class BraveryGloveEntity extends ThrownEntity {
    ...
    public BraveryGloveEntity(EntityType<? extends BraveryGloveEntity> entityType, World world) {
        super(entityType, world);
    }

    @Environment(EnvType.CLIENT)
    public BraveryGloveEntity(World world, double x, double y, double z, int id, UUID uuid) {
        super(UniformSouls.BRAVERY_GLOVE_ENTITY, world);
        setPosition(x, y, z);
    ...
    public void onEntityHit(EntityHitResult entityHitResult) {
        if (!this.world.isClient) {
            ...
            if (entity2 instanceof LivingEntity) {
                ...
                bl2 = entity.damage(UniformSouls.BRAVERY_GLOVE_DAMAGE, 5.0F);
                if (bl2) {
                    if (entity.isAlive()) {
    ...
    public void onCollision(HitResult hitResult) {
        ...
        if (hitResult.getType() != HitResult.Type.ENTITY || !((EntityHitResult)hitResult).getEntity().isPartOf(entity)) {
            if (!this.world.isClient) {
                ...
                areaEffectCloudEntity.setParticleType(UniformSouls.SOUL_BRAVERY);
                ...
                world.addParticle(UniformSouls.FLASH_ORANGE, this.getX(), this.getY(), this.getZ(), 0.0D, 0.0D, 0.0D);
                this.remove();
            }
